...
- **색상별 필터** — 노트 목록 상단 팔레트에서 색상 클릭으로 해당 색 노트만 필터링
- **노트 정렬** — 수정일·생성일·제목 기준 오름차순/내림차순 정렬
- **노트 작성·수정·삭제** — 패널 내에서 직접 편집 가능
- **클립보드로 새 노트** — 새 노트 버튼 우클릭 시 클립보드 텍스트가 본문에 자동 삽입
...
- **테마** — 라이트·다크·시스템 테마 지원, 설정 메뉴에서 즉시 전환
- **오프라인 표시** — 네트워크 오류 시 캐시 노트를 표시하고 주황색 배너로 오프라인 안내, 재연결 버튼 및 30초 자동 재시도 지원
- **패널 위치 자동 초기화** — 모니터 설정 변경 시 패널 위치를 자동으로 초기화, 트레이 메뉴 '위치 초기화'로 수동 초기화 가능
- **세션 만료 감지** — 세션 만료 시 빨간 배너 표시 및 재로그인 안내 다이얼로그
